...
class TrackingService: LifecycleService() {
    ...
    lateinit var curNotificationBuider : NotificationCompat.Builder

    private val runTimeInSec = MutableLiveData<Long>()
    ...
    override fun onCreate() {
        super.onCreate()
        curNotificationBuider = baseNotificationBuilder
        postInitialValues()
    ...
    //When user pause and resume tracking, empty list of latlng will be added to the list
    ...
    private fun startTimer(){
        ...
        CoroutineScope(Dispatchers.Main).launch {
            while (isTracking.value!!){
                // Diff b/w now and timeStarted
                lapTime = System.currentTimeMillis() - timeStarted
                // post new lapTime
                runTimeInMillis.postValue(timeRun + lapTime)
                if (runTimeInMillis.value!! >= lastSecTimeStamp + 1000L){
                    runTimeInSec.postValue(runTimeInSec.value!! + 1)
    ...
    private fun updateNotificationTrackingState(isTracking: Boolean){
        ...
        curNotificationBuider.javaClass.getDeclaredField("mActions").apply {
            isAccessible = true
            set(curNotificationBuider, ArrayList<NotificationCompat.Action>())
        }
        if (!serviceKilled){
            curNotificationBuider = baseNotificationBuilder.addAction(R.drawable.ic_pause_black_24dp, notificationText, pendingIntent)
            notificationManager.notify(NOTIFICATION_ID, curNotificationBuider.build())
        }
    ...
    private fun startForegroundService(){
        ...
        runTimeInSec.observe(this, Observer {
            if (!serviceKilled){
                val notification = curNotificationBuider
                    .setContentText(TrackingUtility.getFormatedTime(it * 1000L))
...
//    // function is used to navigate to tracking fragment whenever user
...
//    )
    ...
    private fun createNotificationChannel(notificationManager: NotificationManager){
        // ANY LEVEL above low will make the phone ring
